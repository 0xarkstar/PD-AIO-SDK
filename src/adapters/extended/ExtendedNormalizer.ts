...
export class ExtendedNormalizer {
  ...
  normalizeMarket(market: ExtendedMarketRaw): Market {
    ...
    const minOrderSize =
      tradingConfig?.minOrderSize || (isApiFormat(market) ? '0' : market.minOrderQuantity || '0');
    const maxLeverage =
      tradingConfig?.maxLeverage || (isApiFormat(market) ? '1' : market.maxLeverage || '1');
    const minPriceChange =
      tradingConfig?.minPriceChange || (isApiFormat(market) ? '0.01' : market.minPrice || '0.01');
  ...
  normalizePosition(position: ExtendedPosition): Position {
    ...
    return {
      ...
      marginRatio:
        size * markPrice > 0 ? safeParseFloat(validated.maintenanceMargin) / (size * markPrice) : 0,
