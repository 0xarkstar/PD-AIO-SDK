 ...
 */
export function normalizeSymbol(katanaSymbol: string): string {
 ...
 */
export function toKatanaSymbol(symbol: string): string {
 ...
 */
export function normalizeMarket(raw: KatanaMarket): Market {
 ...
 */
export function normalizeOrder(raw: KatanaOrder): Order {
  const symbol = normalizeSymbol(raw.market);
 ...
 */
export function normalizePosition(raw: KatanaPosition): Position {
  const symbol = normalizeSymbol(raw.market);
  const qty = parseDecimal(raw.quantity);
  const side: PositionSide = qty >= 0 ? 'long' : 'short';
 ...
 */
export function normalizeBalance(raw: KatanaWallet): Balance {
  return {
 ...
 */
export function normalizeOrderBook(raw: KatanaOrderBook, market: string): OrderBook {
  return {
 ...
 */
export function normalizeTrade(raw: KatanaTrade): Trade {
 ...
 */
export function normalizeFill(raw: KatanaFill): Trade {
  const price = parseDecimal(raw.price);
  const amount = parseDecimal(raw.quantity);
 ...
 */
export function normalizeTicker(raw: KatanaTicker): Ticker {
  return {
    symbol: normalizeSymbol(raw.market),
 ...
 */
export function normalizeFundingRate(raw: KatanaFundingRate): FundingRate {
  return {
    symbol: normalizeSymbol(raw.market),
 ...
 */
export function nonceToUint128(uuidNonce: string): bigint {
...
export function convertOrderRequest(
  ...
  return {
    ...
    triggerPrice:
      request.stopPrice != null ? formatDecimal(request.stopPrice) : KATANA_ZERO_DECIMAL,
...
function toKatanaOrderType(type: OrderType, price?: number): number {
  switch (type) {
    case 'market':
...
function toKatanaTimeInForce(tif?: TimeInForce, postOnly?: boolean): number {
...
function normalizeOrderType(katanaType: number): OrderType {
  const mapped = KATANA_ORDER_TYPE_REVERSE[katanaType];
  return (mapped as OrderType) ?? 'limit';
}

function normalizeOrderSide(katanaSide: number): OrderSide {
  return katanaSide === 0 ? 'buy' : 'sell';
}

function normalizeOrderStatus(katanaState: string): OrderStatus {
  return (KATANA_ORDER_STATUS[katanaState] as OrderStatus) ?? 'open';
}
