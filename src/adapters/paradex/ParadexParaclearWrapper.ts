...
export class ParadexParaclearWrapper {
  ...
  private config?: SDKParadexConfig;
  private provider?: SDKParaclearProvider;
   ...
   * ```
   */
  async initializeAccount(ethersSigner: any): Promise<void> {
    try {
   ...
   * ```
   */
  async checkSocializedLoss(): Promise<{
    active: boolean;
    factor: string;
  }> {
    this.ensureInitialized();
   ...
   * ```
   */
  async calculateReceivableAmount(
    token: string,
    amount: string
  ): Promise<{
