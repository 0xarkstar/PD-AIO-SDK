 ...
 */
export function hyperliquidParseMessage(data: unknown): WebSocketMessage {
  const frame = (typeof data === 'string' ? JSON.parse(data) : data) as HlFrame;
  return {
...
export function hyperliquidResolveKeys(
  ...
  switch (channel) {
    case HYPERLIQUID_WS_CHANNELS.L2_BOOK: {
      const coin = (data as { coin?: string } | undefined)?.coin;
    ...
    case HYPERLIQUID_WS_CHANNELS.USER: {
      const user = (data as { user?: string } | undefined)?.user;
