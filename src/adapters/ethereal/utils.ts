 ...
 */
export function toEtherealOrderSide(side: string): string {
 ...
 */
export function toEtherealOrderType(type: string): string {
 ...
 * Map time in force
 */
export function toEtherealTimeInForce(tif?: string, postOnly?: boolean): string {
...
export function buildOrderRequest(
  ...
  signature: string,
  nonce: string
): EtherealCreateOrderRequest {
  const etherealSymbol = unifiedToEthereal(request.symbol);

  const orderReq: EtherealCreateOrderRequest = {
...
export function mapOrderStatus(
  status: string
): 'open' | 'closed' | 'canceled' | 'rejected' | 'filled' | 'partiallyFilled' {
 ...
 */
export function parseEtherealSymbol(symbol: string): { base: string; quote: string } {
