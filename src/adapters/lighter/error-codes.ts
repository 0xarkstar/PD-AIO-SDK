...
export function extractErrorCode(errorMessage: string): string {
  ...
  // Invalid order
  if (
    messageLower.includes('invalid order') ||
...
export function mapLighterError(
  ...
  // Insufficient Margin
  if (
    errorCode === LIGHTER_CLIENT_ERRORS.INSUFFICIENT_MARGIN ||
  ...
  // Invalid Order
  if (
    errorCode === LIGHTER_CLIENT_ERRORS.INVALID_ORDER ||
  ...
  // Authentication
  if (
    errorCode === LIGHTER_CLIENT_ERRORS.INVALID_SIGNATURE ||
