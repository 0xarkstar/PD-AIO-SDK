...
export class DydxNormalizer {
   ...
   */
  private normalizeOrderType(type: string): Order['type'] {
    switch (type) {
      case 'MARKET':
        return 'market';
      case 'STOP_LIMIT':
   ...
   */
  private normalizeTimeInForce(tif: string): Order['timeInForce'] {
    switch (tif) {
      case 'IOC':
        return 'IOC';
      case 'FOK':
   ...
   */
  normalizePositions(
    positions: Record<string, DydxPerpetualPosition>,
  ...
  normalizeOrderBook(orderBook: DydxOrderBookResponse, ticker: string): OrderBook {
    ...
    const asks: [number, number][] = orderBook.asks.map((level) => [
      parseFloat(level.price),
      parseFloat(level.size),
    ]);
  ...
  normalizeFundingHistory(
    fundingHistory: DydxHistoricalFunding[],
