...
const NOT_IMPLEMENTED_SUFFIX =
  'not implemented — apex adapter is PUBLIC-MARKET-DATA-FIRST (has-flag is false)';

export class ApexAdapter extends BaseAdapter implements IExchangeAdapter {
  readonly id = 'apex';
  ...
  private normalizer: ApexNormalizer;
  private wsHandler?: ApexWebSocketWrapper;
  ...
  private async publicGet<T>(path: string, feature: string, context: string): Promise<T> {
    ...
    return this.unwrapEnvelope<T>(response, context);
  }

  /** Success {data,timeCost} / error {code,msg,timeCost} — both HTTP 200 */
  private unwrapEnvelope<T>(response: unknown, context: string): T {
  ...
  async fetchMarkets(_params?: MarketParams): Promise<Market[]> {
    ...
    const data = await this.publicGet<ApexSymbolsData>('/symbols', 'fetchMarkets', 'symbols');
  ...
  async _fetchTicker(symbol: string): Promise<Ticker> {
    ...
    const data = await this.publicGet<ApexTicker[]>(
  ...
  async _fetchOrderBook(symbol: string, params?: OrderBookParams): Promise<OrderBook> {
    ...
    const data = await this.publicGet<ApexDepthData>(
  ...
  async _fetchTrades(symbol: string, params?: TradeParams): Promise<Trade[]> {
    const apexSymbol = toApexNoDashSymbol(symbol);
    const limit = params?.limit ?? 100;
    const data = await this.publicGet<ApexTrade[]>(
  ...
  async fetchFundingRateHistory(
    ...
    const data = await this.publicGet<ApexHistoryFundingData>(
      path,
  ...
  async fetchOHLCV(
    ...
    if (params?.since) path += `&start=${Math.floor(params.since / 1000)}`;
    if (params?.until) path += `&end=${Math.floor(params.until / 1000)}`;

    const data = await this.publicGet<ApexKlinesData>(path, 'fetchOHLCV', 'klines');
  ...
  private async ensureWebSocketConnected(): Promise<ApexWebSocketWrapper> {
