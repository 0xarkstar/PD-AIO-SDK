 ...
 */
export const APEX_ENDPOINT_WEIGHTS: Record<string, number> = {
  fetchMarkets: 10,
  fetchTicker: 1,
  fetchOrderBook: 5,
  fetchTrades: 5,
 ...
 */
export const APEX_KLINE_INTERVALS: Record<string, string> = {
  '1m': '1',
  '5m': '5',
  '15m': '15',
  '30m': '30',
...
export const APEX_WS_TOPICS = {
  ...
  ORDERBOOK: (symbol: string, depth: 25 | 200 = 200): string => `orderBook${depth}.H.${symbol}`,
  /** recentlyTrade.H.{SYMBOL} */
  TRADES: (symbol: string): string => `recentlyTrade.H.${symbol}`,
  /** instrumentInfo.H.{SYMBOL} — ticker, update-field-only semantics */
  TICKER: (symbol: string): string => `instrumentInfo.H.${symbol}`,
  /** candle.{interval}.{SYMBOL} */
  CANDLE: (interval: string, symbol: string): string => `candle.${interval}.${symbol}`,
} as const;
