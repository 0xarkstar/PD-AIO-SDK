 ...
 * ```
 */
export async function createHmacSha256(key: string, message: string): Promise<string> {
  if (isBrowser) {
    // Web Crypto API
 ...
 * @returns Hex-encoded signature
 */
export function createHmacSha256Sync(key: string, message: string): string {
  if (isBrowser) {
    throw new Error(
      'createHmacSha256Sync is not available in browsers. Use createHmacSha256 instead.'
    );
  }
 ...
 * ```
 */
export async function createSha3Hash(data: string | Uint8Array): Promise<string> {
 ...
 * @returns Hex-encoded hash
 */
export function createSha3HashSync(data: string | Uint8Array): string {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
 ...
 */
export async function createSha3HashBuffer(data: string | Uint8Array): Promise<Uint8Array> {
  const { sha3_256 } = await import('js-sha3');
 ...
 * @returns Hash as Uint8Array
 */
export async function createSha256HashBuffer(data: string | Uint8Array): Promise<Uint8Array> {
  if (isBrowser) {
    const encoder = new TextEncoder();
    const msgData = typeof data === 'string' ? encoder.encode(data) : data;
