...
export class NadoAPIClient {
    ...
    async retryRequest(fn) {
        ...
        throw new ExchangeUnavailableError(`Request failed after ${this.retryConfig.maxAttempts} attempts: ${lastError?.message}`, 'MAX_RETRIES_EXCEEDED', 'nado', lastError);
    }
    /**
    ...
    mapRequestError(error) {
        ...
        if (error.name === 'AbortError' || error.name === 'TimeoutError') {
            return new ExchangeUnavailableError(`Request timeout after ${this.timeout}ms`, 'TIMEOUT', 'nado', error);
        }
        // Network errors
        if (error.code === 'ECONNRESET' ||
            ...
            return new ExchangeUnavailableError(`Network error: ${error.message}`, error.code, 'nado', error);
        }
        // Unknown error
