...
export class ParadexAdapter extends BaseAdapter {
    ...
    requireAuth() {
        if (!this.auth.hasCredentials()) {
            throw new PerpDEXError('Authentication required. Provide apiKey or starkPrivateKey in config.', 'MISSING_CREDENTIALS', 'paradex');
        }
    }
    /**
     * Cleanup resources
    ...
    async _fetchTicker(symbol) {
        ...
        try {
            ...
            const response = await this.client.get(`/markets/summary?market=${market}`);
    ...
    async _fetchTrades(symbol, params) {
        ...
        try {
            ...
            const response = await this.client.get(`/trades?market=${market}&limit=${limit}`);
    ...
    async _fetchFundingRate(symbol) {
        ...
        try {
            const market = this.normalizer.symbolFromCCXT(symbol);
            const response = await this.client.get(`/funding/data?market=${market}&limit=1`);
            const results = response.results;
            if (!Array.isArray(results) || results.length === 0) {
...
//# sourceMappingURL=ParadexAdapter.js.map