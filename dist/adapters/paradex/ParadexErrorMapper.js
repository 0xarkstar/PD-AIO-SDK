...
export function isClientError(code) {
    ...
    if (isNaN(numericCode))
        return false;
    return (Object.values(PARADEX_CLIENT_ERRORS).includes(numericCode) ||
...
export function isServerError(code) {
    ...
    if (isNaN(numericCode))
        return false;
    return (Object.values(PARADEX_SERVER_ERRORS).includes(numericCode) ||
...
export function mapParadexError(errorCode, message, originalError) {
    ...
    if (numericCode === PARADEX_CLIENT_ERRORS.INVALID_SIGNATURE ||
    ...
    // Permission Errors
    if (numericCode === PARADEX_CLIENT_ERRORS.UNAUTHORIZED ||
    ...
    if (numericCode === PARADEX_CLIENT_ERRORS.INSUFFICIENT_MARGIN ||
    ...
    if (numericCode === PARADEX_CLIENT_ERRORS.INVALID_ORDER ||
 ...
 */
export function extractParadexError(response) {
...
export function mapHttpError(status, statusText, responseData) {
    ...
    if (status === 429) {
        ...
        return new RateLimitError(`Rate limit exceeded: ${statusText}`, PARADEX_RATE_LIMIT_ERROR.toString(), 'paradex', retryAfter);
    }
    // 4xx Client Errors
    if (status >= 400 && status < 500) {
        return new InvalidOrderError(`Client error (${status}): ${statusText}`, `HTTP_${status}`, 'paradex');
    }
    // 503 Service Unavailable
    if (status === 503) {
        return new ExchangeUnavailableError(`Service unavailable: ${statusText}`, 'SERVICE_UNAVAILABLE', 'paradex');
    }
    // 5xx Server Errors
    if (status >= 500) {
        return new ExchangeUnavailableError(`Server error (${status}): ${statusText}`, `HTTP_${status}`, 'paradex');
    }
    // Other
...
export function mapAxiosError(error) {
    ...
    if (error.code && isNetworkError(error.code)) {
        return new ExchangeUnavailableError(error.message || 'Network error occurred', error.code, 'paradex', error);
    }
    // Request timeout
    ...
    // Generic error
    return new PerpDEXError(error.message || 'Unknown error occurred', error.code || 'UNKNOWN_ERROR', 'paradex', error);
}
/**
...
//# sourceMappingURL=ParadexErrorMapper.js.map