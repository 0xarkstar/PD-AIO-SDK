 ...
 */
export function mapApexHttpError(status, message) {
    if (status === 403) {
        return new RateLimitError(`${message} (HTTP 403 = ApeX IP ban — back off, do not retry)`, 'IP_BANNED', 'apex');
    }
