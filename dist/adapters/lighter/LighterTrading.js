...
import { toBaseUnits, toPriceUnits, mapOrderType, mapTimeInForce, convertOrderRequest, } from './LighterOrderUtils.js';
/**
 * Create order using WASM signing
 */
export async function createOrderWasm(deps, request) {
 ...
 */
export async function createOrderHMAC(deps, request) {
    const lighterSymbol = deps.normalizer.toLighterSymbol(request.symbol);
 ...
 */
export async function cancelOrderWasm(deps, orderId, symbol) {
    // Get market index
    let marketIndex = 0;
    if (symbol) {
 ...
 */
export async function cancelOrderHMAC(deps, orderId) {
 ...
 */
export async function cancelAllOrdersWasm(deps, symbol) {
 ...
 */
export async function cancelAllOrdersHMAC(deps, symbol) {
 ...
 */
export async function withdrawCollateral(deps, collateralIndex, amount, destinationAddress) {
    if (!deps.signer?.isInitialized || !deps.nonceManager) {
        throw new PerpDEXError('Withdrawals require WASM signing. Configure apiPrivateKey and install @oraichain/lighter-ts-sdk.', 'AUTH_REQUIRED', 'lighter');
    }
    ...
    try {
        ...
        if (response.code !== 0) {
            ...
            await deps.handleTransactionError(response.code);
            throw new PerpDEXError(`Withdrawal failed: code ${response.code}`, 'WITHDRAWAL_FAILED', 'lighter');
        }
...
//# sourceMappingURL=LighterTrading.js.map