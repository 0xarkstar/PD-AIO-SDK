 ...
 */
export function mapDriftError(error) {
    ...
    if (error instanceof Error) {
        ...
        if (message.includes('blockhash') || message.includes('expired')) {
            return new TransactionFailedError('Transaction expired', 'TRANSACTION_EXPIRED', 'drift', undefined, error);
        }
        if (message.includes('signature verification')) {
            return new InvalidSignatureError('Invalid transaction signature', 'INVALID_SIGNATURE', 'drift', error);
        }
        if (message.includes('insufficient lamports') || message.includes('insufficient sol')) {
            return new InsufficientBalanceError('Insufficient SOL for transaction fees', 'INSUFFICIENT_SOL', 'drift', undefined, undefined, error);
        }
        // Check "user account not found" before general "account not found"
        if (message.includes('user account not found')) {
            return new PerpDEXError('Drift user account not initialized', 'USER_NOT_INITIALIZED', 'drift', error);
        }
        ...
        if (message.includes('503') || message.includes('502') || message.includes('504')) {
            return new ExchangeUnavailableError('Solana RPC temporarily unavailable', 'RPC_UNAVAILABLE', 'drift', error);
        }
        ...
        if (message.includes('max number of positions')) {
            return new InvalidOrderError('Maximum number of positions reached', 'MAX_POSITIONS', 'drift', error);
        }
        if (message.includes('market not active')) {
            return new ExchangeUnavailableError('Market is not active', 'MARKET_NOT_ACTIVE', 'drift', error);
        }
        if (message.includes('price bands')) {
            return new InvalidOrderError('Order price outside allowed bands', 'PRICE_BANDS_BREACHED', 'drift', error);
        }
        if (message.includes('order would be filled at worse price')) {
            return new InvalidOrderError('Order would be filled at worse price than limit', 'SLIPPAGE_EXCEEDED', 'drift', error);
        }
    }
