 ...
 * ```
 */
export function mapBackpackError(errorCode, message, originalError) {
    const code = typeof errorCode === 'number' ? errorCode : parseInt(errorCode, 10);
    // Order Errors
    ...
    if (code === BACKPACK_CLIENT_ERRORS.INVALID_ORDER ||
 ...
 */
export function extractBackpackError(response) {
...
export function mapHttpError(status, statusText) {
    // 429 Rate Limit
    if (status === 429) {
        return new RateLimitError(`Rate limit exceeded: ${statusText}`, BACKPACK_RATE_LIMIT_ERROR.toString(), 'backpack', undefined // retryAfter parameter
        );
    }
    // 4xx Client Errors
    if (status >= 400 && status < 500) {
        return new InvalidOrderError(`Client error (${status}): ${statusText}`, `HTTP_${status}`, 'backpack');
    }
    // 5xx Server Errors
    if (status >= 500) {
        return new ExchangeUnavailableError(`Server error (${status}): ${statusText}`, `HTTP_${status}`, 'backpack');
    }
    // Other
