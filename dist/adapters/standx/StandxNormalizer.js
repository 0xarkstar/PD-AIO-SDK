...
import { StandxDepthBookSchema, StandxFundingEntrySchema, StandxKlineHistorySchema, StandxRestTradeSchema, StandxSymbolInfoSchema, StandxSymbolMarketSchema, StandxWSPriceSchema, StandxWSTradeSchema, } from './types.js';
export class StandxNormalizer {
    ...
    normalizeOHLCV(raw) {
        ...
        if (validated.s !== 'ok') {
            throw new PerpDEXError(`Kline history error status: ${validated.s}${validated.errmsg ? ` (${validated.errmsg})` : ''}`, 'INVALID_RESPONSE', 'standx');
        }
        const { t, o, h, l, c, v } = validated;
        if (!t || !o || !h || !l || !c || !v) {
            throw new PerpDEXError('Kline history s:"ok" but column arrays missing', 'INVALID_RESPONSE', 'standx');
        }
        return t.map((seconds, i) => [seconds * 1000, o[i], h[i], l[i], c[i], v[i]]);
    }
}
//# sourceMappingURL=StandxNormalizer.js.map