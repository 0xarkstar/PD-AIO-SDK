 ...
 */
import { ExchangeUnavailableError, InsufficientMarginError, InsufficientBalanceError, InvalidOrderError, InvalidSignatureError, PerpDEXError, PositionNotFoundError, RateLimitError, TransactionFailedError, } from '../../types/errors.js';
 ...
 */
export function mapJupiterError(error) {
    ...
    if (error instanceof Error) {
        ...
        if (message.includes('blockhash') || message.includes('expired')) {
            return new TransactionFailedError('Transaction expired', 'TRANSACTION_EXPIRED', 'jupiter', undefined, error);
        }
        if (message.includes('signature verification')) {
            return new InvalidSignatureError('Invalid transaction signature', 'INVALID_SIGNATURE', 'jupiter', error);
        }
        if (message.includes('insufficient lamports') || message.includes('insufficient sol')) {
            return new InsufficientBalanceError('Insufficient SOL for transaction fees', 'INSUFFICIENT_SOL', 'jupiter', undefined, undefined, error);
        }
        // Pyth-specific errors
        if (message.includes('price not available') || message.includes('no price data')) {
            return new ExchangeUnavailableError('Pyth price data unavailable', 'ORACLE_ERROR', 'jupiter', error);
        }
        ...
        if (message.includes('503') || message.includes('502') || message.includes('504')) {
            return new ExchangeUnavailableError('Solana RPC temporarily unavailable', 'RPC_UNAVAILABLE', 'jupiter', error);
        }
