 ...
 */
import { AVANTIS_PAIR_INDEX_MAP, AVANTIS_INDEX_TO_SYMBOL, PYTH_PRICE_FEED_IDS, } from './constants.js';
/**
 * Get pairIndex from unified symbol
 * @example "BTC/USD:USD" -> 0
 */
export function getPairIndex(symbol) {
 ...
 * @example 0 -> "BTC"
 */
export function getBaseFromPairIndex(pairIndex) {
 ...
 */
export function getPythFeedId(base) {
 ...
 */
export function convertPythPrice(price, expo) {
 ...
 */
export function fromUsdcDecimals(amount) {
 ...
 */
export function toUsdcDecimals(amount) {
    return BigInt(Math.round(amount * 1e6));
}
/**
 * Convert on-chain price (10 decimals) to number
 */
export function fromPriceDecimals(price) {
 ...
 */
export function toPriceDecimals(price) {
    return BigInt(Math.round(price * 1e10));
}
/**
 * Build order params for on-chain transaction
 */
export function buildOrderParams(request, traderAddress) {
