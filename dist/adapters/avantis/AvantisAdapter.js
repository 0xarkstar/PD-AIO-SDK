...
import { AVANTIS_RPC_MAINNET, AVANTIS_RPC_TESTNET, AVANTIS_CONTRACTS_MAINNET, AVANTIS_CONTRACTS_TESTNET, AVANTIS_RATE_LIMIT, AVANTIS_ORDER_TYPES, AVANTIS_TRADING_ABI, AVANTIS_STORAGE_ABI, AVANTIS_PAIR_INFO_ABI, AVANTIS_PYTH_ABI, AVANTIS_ERC20_ABI, AVANTIS_INDEX_TO_SYMBOL, PYTH_PRICE_FEED_IDS, avantisToUnified, unifiedToAvantis, } from './constants.js';
...
export class AvantisAdapter extends BaseAdapter {
    id = 'avantis';
    name = 'Avantis';
    has = {
        // Market Data
    ...
    async initialize() {
        ...
        this.tradingContract = new ethers.Contract(this.contracts.trading, AVANTIS_TRADING_ABI, signerOrProvider);
        this.storageContract = new ethers.Contract(this.contracts.storage, AVANTIS_STORAGE_ABI, signerOrProvider);
        this.pairInfoContract = new ethers.Contract(this.contracts.pairInfo, AVANTIS_PAIR_INFO_ABI, signerOrProvider);
        this.pythContract = new ethers.Contract(this.contracts.pythOracle, AVANTIS_PYTH_ABI, signerOrProvider);
        this.usdcContract = new ethers.Contract(this.contracts.usdc, AVANTIS_ERC20_ABI, signerOrProvider);
        this._isReady = true;
    ...
    requireAuth() {
        if (!this.auth) {
            throw new PerpDEXError('Private key required for authenticated operations', 'MISSING_CREDENTIALS', 'avantis');
        }
        return this.auth;
    }
    requireContract(contract, name) {
        if (!contract) {
            throw new PerpDEXError(`${name} contract not initialized. Call initialize() first`, 'NOT_INITIALIZED', 'avantis');
        }
        return contract;
    }
    // === Public Market Data ===
    ...
    async _fetchOrderBook(_symbol, _params) {
        throw new NotSupportedError('Avantis is an oracle-based DEX with no order book', 'NOT_SUPPORTED', 'avantis');
    }
    async _fetchTrades(_symbol, _params) {
        throw new NotSupportedError('Avantis does not expose public trade history via contracts', 'NOT_SUPPORTED', 'avantis');
    }
    ...
    async fetchFundingRateHistory(_symbol, _since, _limit) {
        throw new NotSupportedError('Avantis funding rate history is not available via on-chain queries', 'NOT_SUPPORTED', 'avantis');
    }
    async fetchOHLCV(_symbol, _timeframe, _params) {
        throw new NotSupportedError('Avantis does not provide OHLCV data on-chain', 'NOT_SUPPORTED', 'avantis');
    }
    // === Private Trading ===
    async createOrder(request) {
        ...
        try {
            ...
            const tx = await openTradeFn([
                ...
                orderParams.tp,
                orderParams.sl,
            ], orderType, slippageP, priceUpdateData);
    ...
    async cancelAllOrders(_symbol) {
        throw new NotSupportedError('Avantis does not support batch cancellation via a single contract call', 'NOT_SUPPORTED', 'avantis');
    }
    // === Account History ===
    async fetchOpenOrders(_symbol, _since, _limit) {
        throw new NotSupportedError('Avantis open limit orders require iterating on-chain storage; use fetchPositions instead', 'NOT_SUPPORTED', 'avantis');
    }
    async fetchOrderHistory(_symbol, _since, _limit) {
        throw new NotSupportedError('Avantis order history is not available via on-chain queries', 'NOT_SUPPORTED', 'avantis');
    }
    async fetchMyTrades(_symbol, _since, _limit) {
        throw new NotSupportedError('Avantis trade history requires event log indexing', 'NOT_SUPPORTED', 'avantis');
    }
    // === Positions & Balance ===
                ...
                try {
                    ...
                    for (let i = 0; i < tradeCount; i++) {
                        try {
                            ...
                            if (!posSize || BigInt(posSize) === BigInt(0))
                                continue;
                            const trade = {
    ...
    async _setLeverage(_symbol, _leverage) {
        throw new NotSupportedError('Avantis sets leverage per-trade at order creation time, not as a separate operation', 'NOT_SUPPORTED', 'avantis');
    }
}
//# sourceMappingURL=AvantisAdapter.js.map