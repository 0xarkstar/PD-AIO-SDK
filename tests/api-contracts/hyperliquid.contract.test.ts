/**
 * Hyperliquid API Contract Tests
 *
 * Validates that the Hyperliquid API conforms to its specification.
 * These tests use real API calls to verify endpoint contracts.
 */

import { describe, it, expect, beforeAll } from '@jest/globals';
import { ContractValidator } from './contract-validator.js';
import { hyperliquidSpec, hyperliquidTestnetSpec } from './specs/hyperliquid.spec.js';
import type { ContractValidationReport } from './types.js';

describe('Hyperliquid API Contract Tests', () => {
  let validator: ContractValidator;
  let report: ContractValidationReport;

  beforeAll(async () => {
    // Use testnet for contract validation
    validator = new ContractValidator(hyperliquidTestnetSpec.baseUrl, {
      timeout: 15000,
    ...
    report = await validator.validateSpecification(hyperliquidTestnetSpec);
  }, 30000); // 30 second timeout for beforeAll

  describe('Contract Validation', () => {
    it('should complete validation for all endpoints', () => {
      // Verify that all endpoints were attempted
      expect(report.results).toHaveLength(hyperliquidTestnetSpec.endpoints.length);
      expect(report.totalEndpoints).toBe(hyperliquidTestnetSpec.endpoints.length);

      // Success rate may vary depending on API availability
    ...
    it('should generate a valid markdown report', () => {
      ...
      expect(markdown).toContain('**Exchange**: hyperliquid-testnet');
  ...
  describe('Individual Endpoint Validation', () => {
    it('should attempt to validate fetchMarkets endpoint', () => {
      const result = report.results.find((r) => r.endpointId === 'hyperliquid.fetchMarkets');

      expect(result).toBeDefined();
      expect(result?.endpointId).toBe('hyperliquid.fetchMarkets');
    ...
    it('should attempt to validate fetchTicker endpoint', () => {
      const result = report.results.find((r) => r.endpointId === 'hyperliquid.fetchTicker');
    ...
    it('should attempt to validate fetchOrderBook endpoint', () => {
      const result = report.results.find((r) => r.endpointId === 'hyperliquid.fetchOrderBook');
    ...
    it('should attempt to validate fetchTrades endpoint', () => {
      const result = report.results.find((r) => r.endpointId === 'hyperliquid.fetchTrades');
    ...
    it('should attempt to validate fetchFundingRate endpoint', () => {
      const result = report.results.find((r) => r.endpointId === 'hyperliquid.fetchFundingRate');
  ...
  describe('Error Handling', () => {
    it('should handle network timeouts gracefully', async () => {
      const timeoutValidator = new ContractValidator(
        hyperliquidTestnetSpec.baseUrl,
        {
      ...
      const timeoutReport = await timeoutValidator.validateSpecification({
        ...
        endpoints: [hyperliquidTestnetSpec.endpoints[0]], // Test one endpoint
      });
    ...
    it('should skip auth-required endpoints when configured', () => {
      ...
      const authEndpointIds = hyperliquidTestnetSpec.endpoints
  ...
  describe('Report Generation', () => {
    it('should include all validation details in report', () => {
      expect(report.exchange).toBe('hyperliquid-testnet');
      expect(report.totalEndpoints).toBe(hyperliquidTestnetSpec.endpoints.length);
